...
JFF::ShaderCodeBuilderPhongGL::ShaderCodeBuilderPhongGL()
{
	JFF_LOG_INFO_LOW_PRIORITY("Ctor ShaderCodeBuilderPhongGL")
}

JFF::ShaderCodeBuilderPhongGL::~ShaderCodeBuilderPhongGL()
{
	JFF_LOG_INFO_LOW_PRIORITY("Dtor ShaderCodeBuilderPhongGL")
}

void JFF::ShaderCodeBuilderPhongGL::generateCode(const Params& params, 
...
inline std::string JFF::ShaderCodeBuilderPhongGL::getShaderVersionLine(const Params& params) const
{
...
inline std::string JFF::ShaderCodeBuilderPhongGL::getVertexShaderCode(const Params& params) const
{
	...
	static std::string code =
		R"glsl(
			...
			void main()
			{
				...
				output.TBN = mat3(tangentWorldSpace, bitangentWorldSpace, output.normalWorldSpace);

				output.uv = uvModelSpace.xy;

				for(int i = 0; i < @1; ++i)
				{
					output.fragPosDirLightSpace[i] = dirLightMatrices[i] * output.fragPosWorldSpace;
				}
				for(int i = 0; i < @3; ++i)
				{
					output.fragPosSpotLightSpace[i] = spotLightMatrices[i] * output.fragPosWorldSpace;
				}
...
inline std::string JFF::ShaderCodeBuilderPhongGL::getFragmentShaderCode(const Params& params) const
{
	...
	static std::string normalMapping =
		R"glsl(
			void normalMapping()
			{
				...
				finalNormalWorldSpace = normalize(input.TBN * normalTangentSpace);
			}
		)glsl";
	...
	static std::string primitiveNormal =
		R"glsl(
			void normalMapping()
			{
				finalNormalWorldSpace = normalize(input.normalWorldSpace);
			}
		)glsl";
	...
	static std::string mainFunctionCode =
		R"glsl(
			...
			void parallaxMapping()
			{
				...
				mat3 TBN_inv = transpose(input.TBN); // Inverse of orthogonal matrix is equal to its transpose, which is way cheaper in computation
				vec3 fragToCamDirWorldSpace = cameraPosWorldSpace - input.fragPosWorldSpace.xyz;
			...
			{
				float diffuseIncidence = max(dot(-normalize(lightDirWorldSpace), finalNormalWorldSpace), 0.0);
				return diffuseIncidence * lightColor * diffuse.xyz;
			}

			vec3 specularFunction(vec3 lightColor, vec3 lightDirWorldSpace)
			{
				vec3 lightReflectionWorldSpace = normalize(reflect(normalize(lightDirWorldSpace), finalNormalWorldSpace));
				...
				vec3 fragPosViewSpace = (viewMatrix * input.fragPosWorldSpace).xyz;
			...
			{
				vec4 fragPosLightSpace = input.fragPosDirLightSpace[index];
			...
			{
				vec3 lightToFragDirWorldSpace = input.fragPosWorldSpace.xyz - pointLights[index].position; // No need to normalize when sampling a cubemap
			...
			{
				vec4 fragPosLightSpace = input.fragPosSpotLightSpace[index];
				vec3 fragPosLightSpaceNDC = fragPosLightSpace.xyz / fragPosLightSpace.w; // From clip space [-w,w] to Normalice Device Coordinates [-1,1]
				fragPosLightSpaceNDC = fragPosLightSpaceNDC * 0.5 + 0.5; // From [-1,1] to [0,1]
			...
			vec3 pointLightsContrib()
			{
				...
				for(int i = 0; i < @2; ++i)
				{
					float dist = length(input.fragPosWorldSpace.xyz - pointLights[i].position);
					...
					vec3 lightDirWorldSpace = normalize(input.fragPosWorldSpace.xyz - pointLights[i].position);
			...
			vec3 spotLightsContrib()
			{
				...
				for(int i = 0; i < @3; ++i)
				{
					float dist = length(input.fragPosWorldSpace.xyz - spotLights[i].position);
					...
					vec3 lightDirWorldSpace = normalize(input.fragPosWorldSpace.xyz - spotLights[i].position);
			...
			{
				vec3 camToFragDirWorldSpace = normalize(vec3(input.fragPosWorldSpace) - cameraPosWorldSpace);
				vec3 reflectionDirWorldSpace = reflect(camToFragDirWorldSpace, finalNormalWorldSpace);
			...
			void main()
			{
				...
				normalMapping();
				FragColor = vec4(directionalLightsContrib() + pointLightsContrib() + spotLightsContrib() + environmentFunction(), opacity.r);
			}
		)glsl";
