...
namespace JFF
{
	...
	private:
		...
		inline std::shared_ptr<Material> generateMaterial(const aiScene* scene, aiMesh* mesh, const std::string& meshName) const;

		inline bool fillMaterialWithTexturesOfType(aiTextureType texType, aiMesh* mesh, aiMaterial* mat, const aiScene* scene,
			std::shared_ptr<Material>& material, std::shared_ptr<MaterialFunctionCodeBuilder>& materialFunctionCodeBuilder) const;
