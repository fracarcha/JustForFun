...
extern std::shared_ptr<JFF::Texture> createTexture(JFF::Engine* const engine, const char* name,
...
{
	return iniFile->getString("model", "path");
}
...
{
	useNormalMap = iniFile->has("config", "use-normal-map") && iniFile->getString("config", "use-normal-map") == "true";
}
...
void JFF::ModelAssimp::extractLocalTransform(aiNode* node, Vec3& localPos, Vec3& localRot, Vec3& localScale)
{
	...
	localRot = Vec3(aiLocalRot.x, aiLocalRot.y, aiLocalRot.z);
...
inline std::shared_ptr<JFF::Material> JFF::ModelAssimp::generateMaterial(const aiScene* scene, aiMesh* mesh, 
...
inline bool JFF::ModelAssimp::fillMaterialWithTexturesOfType(aiTextureType texType, aiMesh* mesh, aiMaterial* mat, 
	const aiScene* scene, std::shared_ptr<Material>& material, std::shared_ptr<MaterialFunctionCodeBuilder>& materialFunctionCodeBuilder) const
{
...
inline void JFF::ModelAssimp::fillMaterialWithConstantsOfType(aiTextureType texType, aiMaterial* mat, 
	std::shared_ptr<MaterialFunctionCodeBuilder>& materialFunctionCodeBuilder) const
{
	...
	std::shared_ptr<Texture>& texture) const
{
	...
	if (tex) // TODO: If this is != nullptr, it means that texture is embedded
	{
		// TODO: Load embedded texture
	}
	else // Texture is external
	{
		img = engine->io.lock()->loadImage(texPath.C_Str());
	}
	...
	*/
	...
	texture = createTexture(engine, texName.c_str(), img, wrapMode, Texture::DEFAULT_FILTER_MODE, isSRGBTexture);
}
