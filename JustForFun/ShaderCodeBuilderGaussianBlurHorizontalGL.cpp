...
JFF::ShaderCodeBuilderGaussianBlurHorizontalGL::ShaderCodeBuilderGaussianBlurHorizontalGL()
{
	JFF_LOG_INFO_LOW_PRIORITY("Ctor ShaderCodeBuilderGaussianBlurHorizontalGL")
}

JFF::ShaderCodeBuilderGaussianBlurHorizontalGL::~ShaderCodeBuilderGaussianBlurHorizontalGL()
{
	JFF_LOG_INFO_LOW_PRIORITY("Dtor ShaderCodeBuilderGaussianBlurHorizontalGL")
}

void JFF::ShaderCodeBuilderGaussianBlurHorizontalGL::generateCode(
	const Params& params, 
...
inline std::string JFF::ShaderCodeBuilderGaussianBlurHorizontalGL::getShaderVersionLine(const Params& params) const
{
...
inline std::string JFF::ShaderCodeBuilderGaussianBlurHorizontalGL::getVertexShaderCode(const Params& params) const
{
...
inline std::string JFF::ShaderCodeBuilderGaussianBlurHorizontalGL::getFragmentShaderCode(const Params& params) const
{
	static std::string code =
		R"glsl(
			...
			void main()
			{
				// Calculate horizontal gaussian blur and store the output in color channel
				vec2 texelSize = 1.0 / textureSize(ppOutputColor, int(mipLevel)); // Texel size (in normalized space) in LOD mipLevel
				vec3 result = textureLod(ppOutputColor, input.uv, mipLevel).rgb * weights[0]; // Sample the current fragment
				
				for (int i = 1; i < NUM_WEIGHTS; ++i) // Sample horizontal neighbours and combine them using gaussian weights
				{
					result += textureLod(ppOutputColor, input.uv + vec2(texelSize.x * i, 0.0), mipLevel).rgb * weights[i];
					result += textureLod(ppOutputColor, input.uv - vec2(texelSize.x * i, 0.0), mipLevel).rgb * weights[i];
				}
