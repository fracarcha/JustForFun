...
inline std::string JFF::ShaderCodeBuilderGaussianBlurHorizontalGL::getFragmentShaderCode(const Params& params) const
{
	static std::string code =
		R"glsl(
			...
			void main()
			{
				// Calculate horizontal gaussian blur and store the jff_output in color channel
				...
				{
					result += textureLod(ppOutputColor, jff_input.uv + vec2(texelSize.x * i, 0.0), mipLevel).rgb * weights[i];
					result += textureLod(ppOutputColor, jff_input.uv - vec2(texelSize.x * i, 0.0), mipLevel).rgb * weights[i];
				}
