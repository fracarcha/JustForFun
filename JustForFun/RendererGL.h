...
namespace JFF
{
	...
	public:
		...
		*/
		...
		/*
		* Select how this renderer will operate. This function will take effect if called before postLoad().
		* * FORWARD: Each light (M) will operate over each object (N). Complexity M x N (GPU greedy)
		* * DEFERRED: All lights will operate in a post process pass. To do this, a bunch of render buffers will be created per frame.
		*			  Complexity M + N (VRAM greedy)
		*/
	...
	protected:
		Engine* engine;
		RenderPath renderPathHint; // This is only a hint. The real active render path is given by activeRenderPath variable
		...
		const int maxPointLightsForwardShading;
		const int maxDirectionalLightsForwardShading;
		const int maxSpotLightsForwardShading;
