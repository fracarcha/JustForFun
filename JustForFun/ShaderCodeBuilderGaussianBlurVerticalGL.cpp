...
inline std::string JFF::ShaderCodeBuilderGaussianBlurVerticalGL::getFragmentShaderCode(const Params& params) const
{
	static std::string code =
		R"glsl(
			...
			void main()
			{
				// Calculate horizontal gaussian blur and store the output in brightness channel
				...
				{
					result += textureLod(ppOutputColor, input.uv + vec2(0.0, texelSize.y * i), mipLevel).rgb * weights[i];
					result += textureLod(ppOutputColor, input.uv - vec2(0.0, texelSize.y * i), mipLevel).rgb * weights[i];
				}
