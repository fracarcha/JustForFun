...
extern std::shared_ptr<JFF::MeshComponent> createMeshComponent(
...
template<>
inline std::weak_ptr<JFF::MeshComponent> JFF::GameObject::addComponent(
	const char* componentName,
	bool initiallyEnabled, 
	const std::shared_ptr<JFF::Mesh>& vertexData)
{
	// Create the Component
	auto comp = createMeshComponent(this, componentName, initiallyEnabled, vertexData);

	// Create a delay loaded function to add the Component to component list
	auto delayLoadedComponentLambda = [this](const std::shared_ptr<MeshComponent>& comp)
	{
		std::string componentName = typeid(MeshComponent).name();
		components.push_back(std::pair<std::string, std::shared_ptr<Component>>(componentName, comp));
	};
	auto delayLoadedComponentFn = std::bind(delayLoadedComponentLambda, comp);
	delayLoadedComponents.push_back(delayLoadedComponentFn);
...
{
	// Create the Component
	auto comp = createMeshComponent(this, componentName, initiallyEnabled, predefinedShape);
